...
- 운영체제에서 오류 메시지를 전송받는 데 주로 사용한다.
- 프로토콜 번호가 1로 할당된다.
- TCP나 UDP와는 달리, 시스템 간 데이터를 교환하지 않고 최종 사용자에 적용되지 않는다.
#### IPsec (Internet Protocol Security)
- 통신 세션의 각 IP패킷을 인증하고 암호화.
#### ARP (Address Resolution Protocol. 주소 결정 프로토콜)
- IP주소를 MAC주소로 대응(bind)
#### RIP (Routing Information Protocol)
- UDP/IP 상에서 동작하는 라우팅 프로토콜
#### BGP (Border Gateway Protocol. 경계 경로 프로토콜)
- 인터넷에서 주 경로 지정을 담당.  
목적지까지 경유하는 자율 시스템(AS) 중 라우팅 및 자율 시스템의 순서를 전송하기 위한 경로 지정 알고리즘.
## 계층 2. 데이터 링크 계층
#### 이더넷
- MAC 패킷과 프로토콜의 형식을 정의한다.  
물리 계층에서는 신호와 배선 형식을 정의한다.
- LAN, MAN, WAN 등에서 모두 사용하며, 가장 많이 활용된다.
#### 토큰링
- 근거리 통신망 기술이지만, 이더넷에 밀렸다.
#### FDDI (Fiber Distributed Data Interface. 광섬유 분산 데이터 접속 방식)
- 근거리 통신망의 데이터 전송 표준.
#### PPP (Point-to-Point Protocol. 점대점 프로토콜)
- 두 통신 노드 간의 직접 연결에 사용.
- 인증, 암호화를 통한 전송 및 데이터 압축 기능.
#### 프레임 릴레이 (Frame Relay)
- LAN 간 혹은 WAN 내 단말 지점 간의 데이저 전송 비용을 효율으로 만든다.
- 가변 길이 단위인 프레임에 데이터를 넣고 재전송 같은 오류 정정 기능은 단말 지점에 맡긴다.  
이를 통해 전체 데이터 전송 속도를 향상시켰다.
#### ATM (Asynchronous Transfer Mode. 비동기 전송 방식)
- 자료를 일정한 크기로 정하여 순서대로 전송하는 자료 전송 방식.
## 계층 1. 물리 계층
#### RS-232 (Recommended Standard 232)
- PC와 음향 커플러, 모뎀 등을 접속하는 직렬 방식의 인터페이스.
- 물리적인 단자의 인터페이스는 포트라고도 하며, 일반적으로 직렬 포트라고 부른다.
#### 100BASE-TX
- 고속 이더넷(Fast Ethernet) 표준 중 가장 흔한 버전이다.  
고속 이더넷은 100 Mbit/s의 평균 속도로 통신을 전달한다.  
최초의 이더넷 속도는 10Mbit/s였다.
- 구리와 광섬유 모두 사용할 수 있다.
#### ISDN (Integrated Service Digital Network. 종합 정보 통신망)
- 모든 정보를 디지털 신호로 만들어 하나의 네트워크를 통해 서비스를 제공하는 통신망
#### DSL (Digital Subscriber Line. 디지털 가입자 회선)
- 지역 전화망을 통해 디지털 데이터 전송을 제공.
- 높은 주파수를 사용한다. (일반 전화는 낮은 주파수를 사용한다.)
- xDSL이라고도 한다.
- HDSL, SDSL, ADSL, VDSL 등의 기술이 있다.
