...
val과 비교하는 것은 mid를 통해서 하지만, 루프를 끝내는 것은 s와 e를 통해서 한다.  
if 문의 조건이 원하는 조건이며, s는 항상 이 때의 mid보다 1 큰 값을 갖는다.  
반면 mid는 else 일 때를 마지막으로 루프가 끝나면, 조건에 맞지 않는 값을 갖는다.  
따라서 아래에서 볼 탐색 위치를 얻을 때도 mid가 아닌 s-1을 사용하는 것이다.
